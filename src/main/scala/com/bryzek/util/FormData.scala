...
object EncodingOptions {
  ...
  /** If specified, when we url encode form data for arrays, we will omit indexes. e.g. "variant_id[0]=bar" ->
    * "variant_id=bar" This is required for the default parameter parsing in play
    */
  case object OmitArrayIndexes extends EncodingOptions
}

/** Convert any url form encoded params (query or body) to a Json object. Makes best guesses on types.
  */
object FormData {

  private val Encoding: String = "UTF-8"

  /** Given a url encoded string, parses it and then reformats as url encoded. Main use case is to turn things like:
    *
  ...
  /** Converts the specified js value into a url form encoded string, recursively through all types.
    *
    * @param keys
    *   Keeps track of the top level keys we are parsing to build up nested keys (e.g. user[first] for maps)
    */
  def toEncoded(
    js: JsValue,
    keys: Seq[String] = Nil,
  ...
  private def mergeObjects(existingObject: JsObject, otherObject: JsObject): JsObject = {
    val result = existingObject.value ++ otherObject.value.map { case (otherKey, otherValue) =>
  ...
  /** Merge two arrays, preserving order where possible. Use case is for when we have two arrays sparsely populated,
    * e.g: one: [1] two: [null, 2] in this case we return [1, 2]
    */
